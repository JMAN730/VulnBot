...
from clawbot.i18n import _, init_i18n
from clawbot.target_state.store import get_target_state_preview, list_target_snapshots
...
def _h_report(session: dict[str, Any], args: str) -> str | None:
    ...
    if not target:
        session["_message"] = _("tui.please_set_target")
        return None
    from clawbot.cli.main import _generate_report_for_target
...
class DashboardScreen(Screen):
    ...
    def _run_subprocess(self, draft: Any, nl_text: str | None = None) -> None:
        ...
        from clawbot.cli.tui import build_command_preview_args

        cmd_args = build_command_preview_args(draft, nl_text=nl_text)
        args = [sys.executable, "-m", "clawbot.cli.main"] + cmd_args[1:]
