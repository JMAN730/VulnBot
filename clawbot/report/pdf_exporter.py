"""PDF export for ClawBot reports.

Spec §5.3: consume ClawBot's existing report (the markdown produced by
``clawbot.report.generator.generate_report``) and render it to a styled PDF —
not a parallel report builder. ``reportlab`` is an optional extra
(``clawbot[pdf]``); importing this module is fine without it, but calling
...
_PDF_EXTRA_HINT = (
    "PDF export requires reportlab. Install the extra with: pip install 'clawbot[pdf]'"
)
...
def export_pdf(
    ...
    """Render a ClawBot markdown report to a PDF file.
