"""
ClawBot MITRE ATT&CK Mapping Engine
=====================================
...
# Maps ClawBot's allowed tools to the ATT&CK techniques they implement.
...
def _build_navigator_layer(
    ...
    layer = {
        ...
        "domain": _NAV_DOMAIN,
        "description": description or f"Generated by ClawBot v{__version__}",
        ...
        "metadata": [
            {"name": "generator", "value": f"ClawBot v{__version__}"},
...
class AttackMapper:
    """Maps ClawBot findings and tool usage to MITRE ATT&CK techniques.
    ...
    def generate_navigator_layer(
        ...
        Args:
            ...
            name: Layer name (default: "ClawBot: <target>")
        ...
        """
        if not name:
            name = f"ClawBot: {report.target}" if report.target else "ClawBot Assessment"
        if not description:
            description = (
    ...
    def format_report(report: AttackReport) -> str:
        ...
        lines.extend([
            ...
            "**Source:** 🔍 Finding | 🔧 Tool",
            f"\n*Generated by ClawBot v{__version__}*",
        ])
