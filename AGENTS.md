...
This is an authorized security-testing tool. Do not commit API keys, target secrets, generated session data, or local config. Document provider setup with commands like `vulnbot config provider deepseek` and `vulnbot config set llm.api_key ...`, not hard-coded values.
