"""VulnBot configuration management - load, save, and access settings."""
...
def ensure_dirs() -> None:
    """Create VulnBot config directories if they don't exist."""
...
def load_config() -> VulnBotConfig:
    ...
    config = VulnBotConfig(mcp=MCPServersConfig(servers=servers))
...
def save_config(config: VulnBotConfig) -> None:
...
def _merge_config(base: VulnBotConfig, raw: dict[str, Any]) -> VulnBotConfig:
...
def _overlay_env(config: VulnBotConfig) -> VulnBotConfig:
    ...
    if v := os.environ.get("VULNBOT_LLM_MODEL"):
        config.llm.model = v
    if v := os.environ.get("VULNBOT_LLM_PROVIDER"):
        config.llm.provider = v
    if v := os.environ.get("VULNBOT_LLM_MAX_TOKENS"):
        with suppress(ValueError):
            config.llm.max_tokens = int(v)
    if v := os.environ.get("VULNBOT_LLM_MAX_CONTEXT_TOKENS"):
        with suppress(ValueError):
            config.llm.max_context_tokens = int(v)
    if v := os.environ.get("VULNBOT_LLM_TEMPERATURE"):
        with suppress(ValueError):
            config.llm.temperature = float(v)

    # Session
    if v := os.environ.get("VULNBOT_SESSION_OUTPUT_DIR"):
        config.session.output_dir = Path(v)
    if v := os.environ.get("VULNBOT_SESSION_AUTO_SAVE"):
        config.session.auto_save = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("VULNBOT_SESSION_REPORT_FORMAT"):
        config.session.report_format = v
    if v := os.environ.get("VULNBOT_SESSION_MAX_ROUNDS"):
        with suppress(ValueError):
    ...
    if v := os.environ.get("VULNBOT_SAFETY_PYTHON_EXECUTE_RESTRICTED"):
        config.safety.python_execute_restricted = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("VULNBOT_SAFETY_PYTHON_EXECUTE_MODE"):
        config.safety.python_execute_mode = v
    if v := os.environ.get("VULNBOT_SAFETY_PYTHON_EXECUTE_MAX_LINES"):
        with suppress(ValueError):
            config.safety.python_execute_max_lines = int(v)
    if v := os.environ.get("VULNBOT_SAFETY_PYTHON_EXECUTE_SHOW_WARNING"):
        config.safety.python_execute_show_warning = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("VULNBOT_SAFETY_PYTHON_EXECUTE_MAX_OUTPUT_CHARS"):
        with suppress(ValueError):
            config.safety.python_execute_max_output_chars = int(v)
    if v := os.environ.get("VULNBOT_SAFETY_PYTHON_EXECUTE_AUDIT_ENABLED"):
...
def apply_provider_preset(config: VulnBotConfig, provider_name: str) -> VulnBotConfig:
